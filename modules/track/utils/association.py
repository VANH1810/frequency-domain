...
from modules.track.utils.iou import AssociationFunction
...
def associate(
    ...
    # assert iou_matrix.ndim == 2, f"iou_matrix must be 2D, got shape {iou_matrix.shape}"
    if min(iou_matrix.shape):
        a = (iou_matrix > iou_threshold).astype(np.int32)
        # print("np.where(a) output:", np.where(a))
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            matched_indices = np.stack(np.where(a), axis=1)
        else:
            if emb_cost is None:
            ...
            matched_indices = np.array(linear_assignment(final_cost)).reshape(-1, 2)
    ...
    # print("matched_indices shape: ", matched_indices.shape)
